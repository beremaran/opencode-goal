...
function assistant(id: string, created: number, text: string): TranscriptMessage {
  return {
    info: {
      id,
      role: "assistant",
...
test("sets, evaluates, continues, and completes a session goal", async (context) => {
  const stateDirectory = await mkdtemp(path.join(tmpdir(), "opencode-goal-plugin-"));
  ...
  assert.equal(
    (config.command as Record<string, { description: string }>).goal?.description,
  ...
  assert.match(output.parts[0]?.text ?? "", /<objective>tests and lint pass<\/objective>/);
  ...
  const replacement = await hooks.tool?.create_goal?.execute({ objective: "documentation is current" }, {
    sessionID: "ses_parent",
    messageID: "asst_3",
  } as never);
...
test("lets an agent create a goal without replacing unfinished work", async (context) => {
  const stateDirectory = await mkdtemp(path.join(tmpdir(), "opencode-goal-agent-create-"));
  context.after(() => rm(stateDirectory, { recursive: true, force: true }));
  ...
  const messages: TranscriptMessage[] = [
    ...
    assistant("asst_create", 2, "I created the goal, but the release is not published yet."),
  ];
  ...
  const rejected = await hooks.tool?.create_goal?.execute({ objective: "silently replace the release goal" }, {
    sessionID: "ses_agent",
    messageID: "asst_replace",
  } as never);
  ...
  assert.match(JSON.stringify(evaluatorPrompts[0]), /I created the goal, but the release is not published yet/);
  assert.equal(continuations.length, 1);
...
test("pauses an active goal when OpenCode reports an interrupted message", async (context) => {
  const stateDirectory = await mkdtemp(path.join(tmpdir(), "opencode-goal-interrupt-"));
  context.after(() => rm(stateDirectory, { recursive: true, force: true }));
