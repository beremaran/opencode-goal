...
test("registers and executes V2 goal tools", async (context) => {
    const stateDirectory = await mkdtemp(path.join(tmpdir(), "opencode-goal-v2-"));
    ...
    assert.deepEqual([...tools.keys()], ["create_goal", "get_goal", "update_goal"]);
    ...
    const store = new FileGoalStore(scopedStateDirectory(stateDirectory, "project_v2", "/workspace"));
...
test("evaluates and completes a V2 goal from idle events", async (context) => {
    const stateDirectory = await mkdtemp(path.join(tmpdir(), "opencode-goal-v2-idle-"));
    context.after(() => rm(stateDirectory, {recursive: true, force: true}));
    ...
    const create = tools.get("create_goal") as {
        execute(input: Record<string, unknown>, toolContext: Record<string, unknown>): Promise<{content: string}>;
    };
