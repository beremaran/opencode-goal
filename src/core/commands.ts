...
async function handlePause(current: GoalState | undefined, store: GoalStore) {
    if (!current || current.status !== "active") {
        return actionPrompt("There is no active goal to pause.");
    }
...
async function handleResume(current: GoalState | undefined, store: GoalStore) {
    if (!current) {
        return actionPrompt("There is no goal to resume.");
    }
...
export async function goalCommandPrompt(
    ...
    if (command.action === "status") {
        return statusPrompt(await store.get(sessionID));
    }
    ...
    if (command.action === "invalid") {
        return actionPrompt(command.message);
    }
    ...
    if (command.action === "pause") {
        return await handlePause(current, store);
    }

    if (command.action === "resume") {
        return await handleResume(current, store);
    }
