...
export function goalMessages(
    ...
    if (startedMessageID) {
        const startedIndex = sorted.findIndex((message) => message.info.id === startedMessageID);
        if (startedIndex >= 0) {
            return sorted.slice(startedIndex);
        }
    }
...
export function totalGoalTokens(messages: TranscriptMessage[], startedAt: number, startedMessageID?: string): number {
    return goalMessages(messages, startedAt, startedMessageID)
        .filter((message) => message.info.role === "assistant")
