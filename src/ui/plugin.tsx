...
import { formatDuration } from "../core/goal.js";
...
function GoalDetails(props: { goal: GoalState; now: number; fg?: TuiColor }) {
  const textProps = props.fg === undefined ? {} : { fg: props.fg };
  return (
    <>
      <text {...textProps}>{snippet(props.goal.objective, 160)}</text>
      <text {...textProps}>
        {props.goal.turns.toLocaleString()} turns · {compactCount(props.goal.tokensUsed)} tokens
      </text>
      <text {...textProps}>{formatDuration(goalElapsedMilliseconds(props.goal, props.now))} elapsed</text>
      <Show when={props.goal.lastReason}>{(reason) => <text {...textProps}>Last: {snippet(reason(), 160)}</text>}</Show>
    </>
  );
}
...
function GoalSidebar(props: { api: TuiPluginApi; sessionID: string; stateRoot: string }) {
      ...
      {(current) => (
        <box>
          <box flexDirection="row" gap={1} onMouseDown={() => setOpen((value) => !value)}>
            ...
            <text fg={statusColor(props.api.theme.current, current().status)}>• {current().status}</text>
          </box>

          <Show when={open()}>
            <GoalDetails goal={current()} now={now()} fg={props.api.theme.current.textMuted} />
          </Show>
        </box>
      )}
    </Show>
...
type V2TuiContext = {
  ...
  data: {
    on(type: string, handler: (event: { type: string; data?: { sessionID?: string } }) => void): () => void;
    session: {
  ...
  ui: {
    router: {
      current(): { type: "session"; sessionID: string } | { type: string; sessionID?: string };
    };
    slot(input: { append: "sidebar.content"; render(props: { sessionID: string }): unknown }): () => void;
  };
};
    ...
    <Show when={goal()}>
      {(current) => (
        <box>
          ...
          <GoalDetails goal={current()} now={now()} />
        </box>
      )}
    </Show>
  );
  ...
  api.slots.register({
    ...
    slots: {
      sidebar_content(_context, props) {
        return <GoalSidebar api={api} sessionID={props.session_id} stateRoot={stateRoot} />;
      },
    },
  });
};
