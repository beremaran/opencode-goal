import { applyEvaluation, applyGoalUpdate, createGoalState, recordGoalTurn, statusPayload } from "../core/goal.js";
import { activeGoalContext, continuationPrompt, EVALUATOR_SYSTEM_PROMPT } from "../core/prompts.js";
...
import { latestAssistant, totalGoalTokens } from "../core/transcript.js";
import type {
  EvaluationDecision,
  GoalState,
  ModelRef,
...
import { evaluateTranscript, parseModelRef } from "./evaluator.js";
...
type V2Context = {
  ...
  tool: {
    transform(callback: (draft: { add(tool: V2ToolDefinition): void }) => void): Promise<unknown>;
  };
};
...
type V2ToolDefinition = {
  ...
  execute(input: Record<string, unknown>, context: V2ToolContext): Promise<V2ToolResult>;
};
...
function liveMessages(trackers: Map<string, TranscriptTracker>, sessionID: string): TranscriptMessage[] {
...
async function sessionStore(
  ...
  return {
    store: new FileGoalStore(scopedStateDirectory(root, session.projectID, session.location.directory)),
    session,
  };
}

function evaluatorModel(configured: string | undefined, session: V2SessionInfo): ModelRef | undefined {
...
async function evaluateGoalV2(
  ...
  try {
    return await evaluateTranscript(goal, messages, options.maxTranscriptChars, async (prompt) => {
    ...
    if (options.deleteEvaluatorSessions && context.session.remove) {
      await context.session.remove({ sessionID: evaluator.id }).catch(() => undefined);
    }
  }
}

async function continueParent(context: V2Context, goal: GoalState, text: string): Promise<void> {
...
async function handleIdle(
  ...
  try {
    ...
    const progress = recordGoalTurn(
      goal,
      assistant.info.id,
      totalGoalTokens(messages, goal.createdAt, goal.startedMessageID),
      messages,
    );
    await store.set(progress);

    const decision = await evaluateGoalV2(context, session, progress, messages, options);
    ...
    if (options.continuationDelayMs > 0) {
      await new Promise<void>((resolve) => setTimeout(resolve, options.continuationDelayMs));
    }
  ...
  await context.tool.transform((tools) => {
    tools.add({
      ...
      async execute(input, toolContext) {
        const { store, session } = await sessionStore(context, toolContext.sessionID, options);
    ...
    tools.add({
      ...
      async execute(input, toolContext) {
        ...
        if (!reason || (status !== "complete" && status !== "blocked")) {
          return result("Goal update rejected: status and reason are required.");
        }
        if (status === "blocked" && goal.turns < 2) {
          return result(`Blocked status rejected: only ${goal.turns + 1} goal turn(s) have run.`);
        }
        if (status === "blocked") {
          const blocked = applyGoalUpdate(goal, "blocked", reason);
          await store.set(blocked);
          return result(statusPayload(blocked));
        }
        const claimed = applyGoalUpdate(goal, "complete", reason);
        await store.set(claimed);
        return result("Completion claim recorded. An independent evaluator will verify it when this turn ends.");
      },
    });
  });
}
  ...
  const consume = async (): Promise<void> => {
    while (!controller.signal.aborted) {
      ...
      if (event.type === "session.idle") {
        ...
        await pauseAfterInterrupt(context, sessionID, options).catch(() => undefined);
      } else if (event.type === "session.deleted") {
        const { store } = await sessionStore(context, sessionID, options).catch(() => ({ store: undefined }));
