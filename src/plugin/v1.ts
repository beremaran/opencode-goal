...
import {goalCommandPrompt} from "../core/commands.js";
import {parseGoalCommand, resolveOptions} from "../core/options.js";
import {activeGoalContext, continuationPrompt} from "../core/prompts.js";
import {defaultStateRoot, FileGoalStore, scopedStateDirectory} from "../storage/goal-store.js";
import {latestAssistant, latestUserExecution, totalGoalTokens} from "../core/transcript.js";
import type {GoalState, ResolvedGoalPluginOptions, TranscriptMessage} from "../core/types.js";
...
async function continueParent(
    ...
    try {
        ...
        if (isParentBusy(status.data, goal.sessionID)) {
            await log("debug", "Skipped continuation because the parent session is busy", {
...
async function handleIdle(input: {
    ...
    try {
        ...
        if (decision.complete) {
            ...
            await showToast(input.client, `Goal complete: ${updated.lastReason}`, "success");
            return;
        }

        const continuing = updated;
        if (input.options.continuationDelayMs > 0) {
            await new Promise<void>((resolve) => setTimeout(resolve, input.options.continuationDelayMs));
        }
...
const v1GoalPlugin: Plugin = async (input, rawOptions) => {
    ...
    const stateDirectory = scopedStateDirectory(root, input.project.id, input.directory);
            ...
            create_goal: tool({
                ...
                args: {
                    objective: tool.schema
                        ...
                        .trim()
                        .min(1)
                        .describe("The concrete condition that will make the goal complete"),
                },
                async execute(args, context) {
                    ...
                    await showToast(input.client, `Goal created by agent: ${goal.objective.slice(0, 120)}`, "info");
            ...
            update_goal: tool({
                ...
                args: {
                    ...
                    reason: tool.schema
                        .string()
                        .min(1)
                        .describe("Concise evidence for completion or the exact repeated blocker"),
                },
                async execute(args, context) {
